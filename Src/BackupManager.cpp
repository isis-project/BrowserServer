...
BackupManager::BackupItem::BackupItem( const std::string& appId,
										BackupType eType, int recursive, double version ) :
	m_appid(appId)
	, m_eType(eType)
...
{
	BackupItem lunaSysMgrCookies(strPhonyCookieAppId, 
				BackupHtml5Db, 0 /*recursive*/, 1.0);
...
bool BackupManager::registerForBackup()
{
	...
	bool succeeded = LSCall(m_clientService, "palm://com.palm.service.backup/register", json_object_get_string(payload),
