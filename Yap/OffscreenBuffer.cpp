...
void OffscreenBuffer::copyFromBuffer(uint32_t* srcBuffer, int srcStride,
									 ...
									 int srcSizeWidth, int srcSizeHeight)
{
	...
	OffscreenRect srcRect(srcPositionX, srcPositionY,
						  srcPositionX + srcSizeWidth,
						  srcPositionY + srcSizeHeight);
	OffscreenRect dstRect(info->scrollX, info->scrollY,
						  info->scrollX + info->width,
						  info->scrollY + info->height);
	...
	src = srcBuffer + (srcRect.top - srcPositionY) * srcStride +
		  (srcRect.left - srcPositionX);
	dst = m_buffer + (srcRect.top - info->scrollY) * info->stride +
		  (srcRect.left - info->scrollX);
...
void OffscreenBuffer::copyToBuffer(uint32_t* dstBuffer, int dstStride,
								   ...
								   int dstSizeWidth, int dstSizeHeight)
{
	...
	OffscreenRect srcRect(info->scrollX, info->scrollY,
						  info->scrollX + info->width,
						  info->scrollY + info->height);
	OffscreenRect dstRect(dstPositionX, dstPositionY,
						  dstPositionX + dstSizeWidth,
						  dstPositionY + dstSizeHeight);
...
//		   srcRect.top, srcRect.bottom);
	...
	src = m_buffer + (srcRect.top - info->scrollY) * info->stride +
		  (srcRect.left - info->scrollX);
	dst = dstBuffer + (srcRect.top - dstRect.top) * dstStride +
...
static void PrvScale(uint32_t* src, int srcWidth, int srcHeight, int srcStride,
					 uint32_t* dst, int dstWidth, int dstHeight, int dstStride)
{
	uint32_t  xinc, yinc;
	uint32_t  *sp, *sptr;
	uint32_t  *dp, *dptr;
...
void OffscreenBuffer::scaleToBuffer(uint32_t* dstBuffer, int dstStride,
									...
									double scale)
{
	...
	OffscreenRect bufferRect(info->scrollX, info->scrollY,
							 info->scrollX + info->width,
							 info->scrollY + info->height);
	...
	OffscreenRect dstScaledRect((int) (srcRect.left * scale),
								(int) (srcRect.top * scale),
								(int) (srcRect.right * scale),
								(int) (srcRect.bottom * scale));
	...
	src = m_buffer + (srcRect.top - info->scrollY) * info->stride +
		  (srcRect.left - info->scrollX);
	dst = dstBuffer + (dstScaledRect.top - dstTop) * dstStride +
		  (dstScaledRect.left - dstLeft);

	PrvScale(src, srcRect.right - srcRect.left,
			 srcRect.bottom - srcRect.top, info->stride,
			 dst, dstScaledRect.right - dstScaledRect.left,
			 dstScaledRect.bottom - dstScaledRect.top, dstStride);
}
...
int OffscreenBuffer::writeToFile(FILE* pFile, int viewLeft, int viewTop, int viewRight, int viewBottom) const
{
	...
	if ( !nErr ) {
		if (setjmp(png_jmpbuf(pPngImage))) {
			...
			png_set_IHDR(pPngImage, pPngInfo, nImgWidth, nImgHeight,
	...
	if (!srcRect.empty() && !nErr) {
		...
		uint32_t* src = m_buffer + (srcRect.top - info->scrollY) * info->stride +
						(srcRect.left - info->scrollX);
